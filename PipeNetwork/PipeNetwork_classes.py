...
#region fundamental classes used in PipeNetwork_model
class Fluid:
...
#region model-view-controller pattern
class PipeNetwork_model():
    def __init__(self, Pipes=None, Loops=None, Nodes=None, fluid=Fluid(), SI=True):
        ...
        From previous examples, we just had a PipeNewtork class.  This is now named
        PipeNetwork_model and a few details of the functions have changed.  It should not
        directly output/input to/from a user interface.
...
class PipeNetwork_view():
    def __init__(self):
        '''
        The PipeNetwork_view is used to output/input information contained in the PipeNetwork_model
        '''
        pass

    def setWidgets(self,args):
        '''
        This function receives a tuple of QtWidgets for output/input from the GUI.
        :param args: tuple of QtWidgets.  Keep same names as GUI widgets to avoid confusion.
        :return: nothing
        '''
        #unpack args
        self.tree_Pipes, self.tree_Nodes, self.tree_Loops, self.lbl_FlowRates, self.lbl_NodePressures, self.lbl_PipeHeadLosses, self.lbl_PressureAndFlowChecks=args
        pass

    #region Functions to display on GUI
    def update(self, PN=None):
        # update the pipe tree
        self.updatePipeTree(self.tree_Pipes, PN=PN)
        # update the node tree
        self.updateNodeTree(self.tree_Nodes, PN=PN)
        # update the loop tree
        self.updateLoopTree(self.tree_Loops, PN=PN)
...
class PipeNetwork_controller():
    def __init__(self):
        '''
        The PipeNetwork_controller coordinates the information flow between the model
        and the view.  It holds a self.View and self.Model for this purpose.
        '''
        self.model = PipeNetwork_model()
        self.view = PipeNetwork_view()

    def setWidgets(self, args):
        self.view.setWidgets(args)
    ...
    def EvaluatePipeNetwork(self):
        ...
        self.view.getPipeFlowRatesOutput(tabs='\t', PN=self.model)
        self.view.getPipeHeadLossesOutput(tabs='\t', PN=self.model)
        self.view.getNodeHeadOutput(tabs='\t', PN=self.model)
        self.view.getRealityCheckOutput(tabs='\t', PN=self.model)
        pass

    def UpdateView(self):
        self.view.update(PN=self.model)
    ...
    def buildNodes(self, PN=None):
        if (PN is None):
            PN=self.model
        PN.buildNodes()

    def getNode(self, name, PN=None):
        if(PN is None):
            PN = self.model
        return PN.getNode(name)
    def getPipe(self, name, PN=None):
        if(PN is None):
            PN = self.model
        return PN.getPipe(name)
    def getLoop(self, name, PN=None):
        if(PN is None):
            PN = self.model
        return PN.getLoop(name)
    def findFlowRates(self, PN=None):
        if(PN is None):
            PN = self.model
        PN.findFlowRates()
    def setRefPressure(self, pres, nodeName, PN=None):
        if(PN is None):
            PN = self.model
        PN.setRefPressure(pres, nodeName)
    # endregion
    ...
    #region Functions for output
    def printPipeFlowRates(self, SI=True, PN=None):
        if PN is None:
            PN=self.model
        self.view.printPipeFlowRates(SI=SI, PN=PN)
    def printNetNodeFlows(self, SI=True, PN=None):
        if PN is None:
            PN=self.model
        self.view.printNetNodeFlows(SI=SI, PN=PN)
    def printLoopHeadLoss(self, SI=True, PN=None):
        if PN is None:
            PN = self.model
        self.view.printLoopHeadLoss(SI=SI, PN=PN)
    def printPipeHeadLoss(self, SI=True, PN=None):
        if PN is None:
            PN = self.model
        self.view.printPipeHeadLoss(SI=SI, PN=PN)
    def printNodeHead(self, SI=True, PN=None):
        if PN is None:
            PN = self.model
        self.view.printNodeHead(SI=SI, PN=PN)
    #endregion
#endregion
#endregion
...
def mainX2():
    ...
    PNM=PNC.model # this is the model
    PNV=PNC.view  # this is the view
    #add Pipe objects to the pipe network
    PNC.addPipe(Pipe('a','b',1000, 18, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('a','h',1600, 24, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('b','c',500, 18, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('b','e',800, 16, r_CI, water, SI=SIUnits))
    PNC.addPipe(Pipe('c','d',500, 18, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('c','f',800, 16, r_CI, water, SI=SIUnits))
    PNC.addPipe(Pipe('d','g',800, 16, r_CI, water, SI=SIUnits))
    PNC.addPipe(Pipe('e','f',500, 12, r_CI, water, SI=SIUnits))
    PNC.addPipe(Pipe('e','i',800, 18, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('f','g',500, 12, r_CI, water, SI=SIUnits))
    PNC.addPipe(Pipe('g','j',800, 18, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('h','i',1000, 24, r_CN, water, SI=SIUnits))
    PNC.addPipe(Pipe('i','j',1000, 24, r_CN, water, SI=SIUnits))

    #add Node objects to the pipe network
    PNC.buildNodes()
    #update the external flow of inlet node
    PNC.modifyNode(PN=PNM, name='h', ext_flow=10)
    PNC.modifyNode(PN=PNM, name='e', ext_flow=-3)
    PNC.modifyNode(PN=PNM, name='f', ext_flow=-5)
    PNC.modifyNode(PN=PNM, name='h', ext_flow=-2)
    PNC.getNode('h').setExtFlow(10,SI=SIUnits)
    #set external flows at required nodes
    PNC.getNode('e').setExtFlow(-3, SI=SIUnits)
    PNC.getNode('f').setExtFlow(-5, SI=SIUnits)
    PNC.getNode('d').setExtFlow(-2, SI=SIUnits)

    #add Loop objects to the pipe network
    PNC.addLoop(Loop('A',[PNC.getPipe('a-b'),PNC.getPipe('b-e'), PNC.getPipe('e-i'),PNC.getPipe('h-i'), PNC.getPipe('a-h')]))
    PNC.addLoop(Loop('B',[PNC.getPipe('b-c'), PNC.getPipe('c-f'),PNC.getPipe('e-f'), PNC.getPipe('b-e')]))
    PNC.addLoop(Loop('C',[PNC.getPipe('c-d'), PNC.getPipe('d-g'),PNC.getPipe('f-g'), PNC.getPipe('c-f')]))
    PNC.addLoop(Loop('D',[PNC.getPipe('e-f'),PNC.getPipe('f-g'), PNC.getPipe('g-j'),PNC.getPipe('i-j'), PNC.getPipe('e-i')]))

    #call the findFlowRates method of the PN (a PipeNetwork object)
    PNC.findFlowRates()
    pRef=water.psi_to_m(80) #reference pressure in m of water
    PNC.setRefPressure(pRef,'h')  #set reference pressure at node 'h' and increase all other nodes too.

    #get output for zero height nodes
    SIUnits=False
    PNC.printPipeFlowRates(SI=SIUnits)
    print('')
    print('Check node flows:')
    PNC.printNetNodeFlows(SI=SIUnits)
    print('')
    print('Check loop head loss:')
    PNC.printLoopHeadLoss(SI=SIUnits)
    print('')
    PNC.printPipeHeadLoss(SI=SIUnits)
    print('')
    PNC.printNodeHead(SI=SIUnits)
#endregion
